...
✅ **AI分析コメント生成** 🆕 **Gemini 2.0 Flash動画分析**
- **Gemini 2.0 Flash Video API**: 動画を実際にダウンロードして分析（カット割り、色彩、構成、演出）
- **実際の映像内容を分析**: TikTok/Instagram動画をGemini APIで解析
- **8セクション詳細分析**: パフォーマンス評価 + 編集分析 + ストーリー構成 + テキスト戦略 + ターゲット推定 + 成功要因 + 改善提案 + アクションプラン（2000-2500文字）
- **自動フォールバック**: Gemini Video → GPT-4o (text) → Cloudflare AI の3段階
...
| **Google Gemini 2.0 Flash API** 🆕 | 動画映像の実分析 | $0.15/1M入力トークン（1分動画≈$0.0023、60秒TikTok≈0.23円） | ⭐ 最推奨・超低コスト |
| **OpenAI API (GPT-4o text)** | 説明文のみAI分析 | $2.50/1M入力、$10.00/1M出力（1動画≈$0.03-0.05） | ⭕ 推奨 |
...
### 本番環境（Gemini 2.0 Flash動画分析版 🆕）
- **Production URL**: https://0db49072.comet-analyzer.pages.dev
- **プロジェクト**: comet-analyzer
- **プラットフォーム**: Cloudflare Pages
- **最終デプロイ**: 2025-01-17 (Gemini 2.0 Flash Video統合 - 実動画ダウンロード＆分析)
...
3. **Cloudflareで環境変数を設定**
   ...
   - `GEMINI_API_KEY`: Google Gemini APIキー（動画分析を使う場合） 🆕 **推奨**
   - `OPENAI_API_KEY`: OpenAI APIキー（GPT-4oテキスト分析を使う場合）
...
| N | 分析結果 | **GPT-4o Vision生成**の超詳細分析（約2000-2500文字）<br>① パフォーマンス評価<br>② 編集・構成分析（カット割り・テンポ・色彩）<br>③ ストーリー構成分析（フック・本編・結末）<br>④ 台本・演出分析（説明文・ハッシュタグ戦略）<br>⑤ ターゲット層推定<br>⑥ 成功要因の統合分析<br>⑦ 改善提案（編集・構成・台本）<br>⑧ アクションプラン（3-5項目） |
| O | メモ/タグ | ユーザー用の自由記入欄 |
...
- **AI分析（3段階）**: 
  - **Tier 1**: OpenAI GPT-4o Vision API（映像+説明文分析・最高品質）🆕
  - **Tier 2**: OpenAI GPT-4o（説明文のみ分析・高品質）
...
## 🤖 GPT-4o Vision AI分析の特徴

### 🎬 動画の映像を実際に見て分析

従来のテキストのみの分析と異なり、**GPT-4o Vision API**は動画を実際に視聴して分析します：
...
#### Gemini Video API使用時（映像分析）🆕 **超低コスト！**
- **1動画あたり（60秒）**: 約$0.0023（0.23円）
- **10動画**: 約$0.023（2.3円）
- **100動画**: 約$0.23（23円）
- **200動画/日**: 約$0.46/日（$13.8/月、1,380円/月）
...
※ Gemini 2.0 Flash: $0.15/1M入力トークン（動画は258トークン/秒@1fps = 15,480トークン/分）  
※ GPT-4o Text: 入力$2.50/1M、出力$10.00/1M

### 🔄 3段階自動フォールバック

1. **Gemini 2.0 Flash Video API**（最高品質・超低コスト）- 動画を実際にダウンロードして分析
2. **GPT-4o Text API**（高品質）- 説明文のみ分析
3. **Cloudflare Workers AI**（標準品質）- 数値のみ分析

Gemini APIキーが未設定の場合、自動的にGPT-4oまたはCloudflare AIにフォールバックします。
...
✅ 指標自動計算（各種率の算出）  
✅ **GPT-4o Vision完全AI分析** 🆕  
  - OpenAI GPT-4o Vision API による動画映像の実視聴分析
  - 説明文・ハッシュタグ戦略の評価
  - カット割り・テンポ・色彩・構成・演出の詳細分析
  - 3段階自動フォールバック（Vision → Text → Cloudflare AI）
...
- 動画映像を実際に視聴して分析（カット割り、色彩、構成、演出）
- 説明文・ハッシュタグ戦略の評価
- 業界基準との比較、成功要因分析、改善提案、アクションプラン
- 3段階自動フォールバック（Vision → Text → Cloudflare AI）
