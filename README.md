...
2. 取得件数を選択
   - **単発実行**: 2-3動画（Cloudflare Workers制限対応）
   - **連続実行**: 2動画 × 20回 = 40動画（推奨）
3. 「Apifyで自動収集」ボタンをクリック
4. **メリット**: ボタン1つで完全自動、Comet不要
5. **注意**: 無料プランは1回あたり2-3動画まで
...
3. **実行モードを選択**
   - **単発実行**: 2-3動画（テスト用・少量収集）
   - **連続実行**: 2動画 × N回（大量収集）
     - 例: 2動画 × 20回 = 40動画
     - ブラウザを開いたまま自動的に繰り返し実行

4. **取得件数を選択**
   - 単発: 1-3動画（Cloudflare Workers無料プラン制限）
   - 連続: 1回あたり2動画 × 実行回数（10-100回）
...
**⚠️ Cloudflare Workers制限について:**
- 無料プランは50サブリクエスト/リクエストの制限があります
- 1動画あたり15-20サブリクエストが必要
- 安全に処理できるのは2-3動画/リクエストまで
- 大量データ収集には**連続実行モード**を使用してください
...
- **処理時間**: 1リクエストあたり2-3分
- **⚠️ Cloudflare制限**: 無料プランは2-3動画/リクエスト
  - 大量データは**連続実行モード**を使用（2動画 × N回）
...
### Cloudflare Workers制限 ⚠️ **重要**

**無料プランの制限:**
- **サブリクエスト数**: 最大50回/リクエスト
- **推奨バッチサイズ**: **2-3動画/リクエスト**
- **有料プラン**: 1000回/リクエスト（$5/月〜）

**1動画あたりのサブリクエスト数:**
1. Apifyからメタデータ取得: 1-2回
2. 動画ファイルダウンロード: 1回
3. Twelve Labs API: 2-3回
4. Google Sheets API: 3-5回
**合計: 約15-20サブリクエスト/動画**

**対策:**
- ✅ デフォルト設定: 2動画/リクエスト（安全）
- ✅ 最大: 3動画/リクエスト（無料プラン上限）
- ✅ 大量データ収集: **連続実行モード**を使用
  - 2動画 × 20回 = 40動画（自動的に分割実行）
  - ブラウザを開いたまま待つだけ
